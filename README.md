...
本專題開發了一套基於 **傳統電腦視覺（OpenCV）** 的高效率、全畫面不分區自動硬幣辨識與時序追蹤系統。在**不依賴任何深度學習模型與標註資料（No AI Training Required）**的前提下，利用純幾何幾何特徵、局部對比增強與時間軸狀態機，成功克服了極端複雜的鍵盤文字紋路干擾、環境光學眩光（Glare）、手部遮擋（Occlusion）以及動態閃爍痛點，完美適配嵌入式系統與 SoC 晶片的低功耗、低延遲實時運算規範。
